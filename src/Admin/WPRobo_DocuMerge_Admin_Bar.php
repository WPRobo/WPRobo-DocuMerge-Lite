...
class WPRobo_DocuMerge_Admin_Bar {
    ...
    public function wprobo_documerge_init_hooks() {
        ...
        add_action( 'wp_head', array( $this, 'wprobo_documerge_admin_bar_css' ) );
        add_action( 'admin_head', array( $this, 'wprobo_documerge_admin_bar_css' ) );
    }
    ...
    private function wprobo_documerge_detect_forms_on_page() {
        ...
        // 1. Shortcode detection: [documerge_form id="X"]
        if ( has_shortcode( $content, 'documerge_form' ) ) {
            if ( preg_match_all( '/\[documerge_form\s[^\]]*id=["\']?(\d+)["\']?/i', $content, $matches ) ) {
    ...
    /**
     * Output minimal CSS for the admin bar DocuMerge icon.
     *
     * @since 1.0.0
     */
    public function wprobo_documerge_admin_bar_css() {
        ...
        ?>
        <style>
            #wp-admin-bar-wprobo-documerge > .ab-item .ab-icon {
                margin-right: 4px;
                top: 2px;
            }
            #wp-admin-bar-wprobo-documerge-separator > .ab-item {
                padding: 0 !important;
