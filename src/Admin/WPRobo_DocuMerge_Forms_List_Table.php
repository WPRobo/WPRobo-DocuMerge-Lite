...
class WPRobo_DocuMerge_Forms_List_Table extends \WP_List_Table {
	...
	public function column_shortcode( $item ) {
		$shortcode = '[documerge_form id="' . absint( $item->id ) . '"]';
	...
	public function prepare_items() {
		...
		// Sort.
		$allowed_orderby = array( 'id', 'title' );
		// phpcs:ignore WordPress.Security.NonceVerification.Recommended
		...
		// Fetch forms with template name.
		// phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
		$select_query = "SELECT f.*, t.name AS template_name
			FROM {$forms_table} f
			LEFT JOIN {$templates_table} t ON f.template_id = t.id
			WHERE 1=1{$where}
