...
class WPRobo_DocuMerge_Forms_Page {
    ...
    public function wprobo_documerge_ajax_create_form_page() {
        ...
        $page_id = wp_insert_post( array(
            ...
            'post_content' => '<!-- wp:shortcode -->[documerge_form id="' . $form_id . '"]<!-- /wp:shortcode -->',
            'post_status'  => 'draft',
            'post_type'    => 'page',
        ) );
    ...
    private function wprobo_documerge_process_bulk_action() {
        ...
        $ids = isset( $_GET['form_ids'] ) ? array_map( 'absint', (array) $_GET['form_ids'] ) : array(); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput
    ...
    public function wprobo_documerge_ajax_save_form() {
        ...
        // Fields — decode JSON, sanitize each field object.
        $fields_raw = isset( $_POST['fields'] )
            ? wp_unslash( $_POST['fields'] ) // phpcs:ignore WordPress.Security.ValidatedSanitizedInput
            : '[]';
        ...
        // Settings — decode JSON, sanitize each setting.
        ...
        $delivery_methods_raw = isset( $_POST['delivery_methods'] )
            ? wp_unslash( $_POST['delivery_methods'] ) // phpcs:ignore WordPress.Security.ValidatedSanitizedInput
            : '[]';
    ...
    public function wprobo_documerge_ajax_preview_document() {
        ...
        if ( is_wp_error( $pdf_path ) ) {
            ...
            wp_send_json_success( array( 'url' => $url, 'format' => 'docx' ) );
            return;
        }
        ...
        wp_send_json_success( array( 'url' => $url, 'format' => 'pdf' ) );
    }
    ...
    public function wprobo_documerge_ajax_serve_preview() {
        ...
        $paths      = array(
            $upload_dir['basedir'] . '/documerge-docs/0/' . $file,
            $upload_dir['basedir'] . '/documerge-docs/' . gmdate( 'Y' ) . '/' . gmdate( 'm' ) . '/0/' . $file,
            $upload_dir['basedir'] . '/documerge-temp/' . $file,
        );

        if ( defined( 'WPROBO_DOCUMERGE_TEMP_DIR' ) ) {
            $paths[] = WPROBO_DOCUMERGE_TEMP_DIR . $file;
        }
