...
class WPRobo_DocuMerge_Submissions_Page {
    ...
    public function wprobo_documerge_init_hooks() {
        ...
        add_action( 'wp_ajax_wprobo_documerge_update_payment_status', array( $this, 'wprobo_documerge_ajax_update_payment_status' ) );
    }
    ...
    private function wprobo_documerge_process_bulk_action() {
        ...
        $ids = isset( $_GET['submission_ids'] ) ? array_map( 'absint', (array) $_GET['submission_ids'] ) : array(); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput
    ...
    public function wprobo_documerge_ajax_delete_submission() {
        ...
        $raw_ids = isset( $_POST['ids'] ) ? $_POST['ids'] : array(); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput
    ...
    /**
     * AJAX handler — update payment status of a submission.
     *
     * Allows admins to manually change the payment status (e.g. mark
     * a stuck "pending" as "paid", or record a refund). Also updates
     * the submission status accordingly.
     *
     * @since 1.0.0
     */
    public function wprobo_documerge_ajax_update_payment_status() {
        check_ajax_referer( 'wprobo_documerge_admin', 'nonce' );
        ...
        $allowed_statuses = array( 'none', 'pending', 'paid', 'failed', 'refunded' );
        if ( ! in_array( $payment_status, $allowed_statuses, true ) ) {
            wp_send_json_error( array( 'message' => __( 'Invalid payment status.', 'wprobo-documerge' ) ) );
            return;
        }
        ...
        // Automatically update the submission status based on payment change.
        if ( 'paid' === $payment_status ) {
            // If payment is marked as paid and submission was pending_payment, move to processing.
            ...
            if ( 'pending_payment' === $current || 'payment_failed' === $current ) {
        ...
        if ( false === $updated ) {
            wp_send_json_error( array( 'message' => __( 'Failed to update payment status.', 'wprobo-documerge' ) ) );
            return;
        }

        $response_data = array(
            'message'        => __( 'Payment status updated.', 'wprobo-documerge' ),
            'payment_status' => $payment_status,
        );

        // When marking as paid, trigger document generation + delivery if not already generated.
        if ( 'paid' === $payment_status ) {
            $submission = $wpdb->get_row(
                $wpdb->prepare(
            ...
            if ( $submission ) {
                $has_document = ! empty( $submission->doc_path_pdf ) || ! empty( $submission->doc_path_docx );

                // Generate document if not already generated.
                if ( ! $has_document ) {
                    $generator  = new \WPRobo\DocuMerge\Document\WPRobo_DocuMerge_Document_Generator();
                    $gen_result = $generator->wprobo_documerge_generate( $submission_id );

                    if ( is_wp_error( $gen_result ) ) {
                        $response_data['message']   = __( 'Payment updated but document generation failed: ', 'wprobo-documerge' ) . $gen_result->get_error_message();
                        $response_data['doc_error'] = true;
                ...
                // Deliver document (download link + email + media library).
                if ( $has_document ) {
                    ...
                    if ( ! is_wp_error( $deliver_result ) ) {
                        ...
                        $response_data['message'] = __( 'Payment updated, document delivered. Email sent to submitter.', 'wprobo-documerge' );
                        $response_data['status']  = 'completed';
                        $response_data['reload']  = true;
                    } else {
                        $response_data['message'] = __( 'Payment updated, but delivery failed: ', 'wprobo-documerge' ) . $deliver_result->get_error_message();
                    }
                }
            }
        }
