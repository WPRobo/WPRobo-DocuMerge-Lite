<?php
/**
 * Dropdown field type for WPRobo DocuMerge form builder.
 *
...
/**
 * Class WPRobo_DocuMerge_Field_Dropdown
 *
 * Handles dropdown select fields within the DocuMerge form builder.
 *
 * @since 1.0.0
 */
class WPRobo_DocuMerge_Field_Dropdown {
	...
	public function wprobo_documerge_get_label() {
		return __( 'Dropdown', 'wprobo-documerge' );
	}
	...
	public function wprobo_documerge_render_frontend( $field_data, $value = '' ) {
		...
		$placeholder = ! empty( $field_data['placeholder'] ) ? esc_html( $field_data['placeholder'] ) : esc_html__( 'Select...', 'wprobo-documerge' );
		...
		foreach ( $options as $option ) {
			...
			$html .= '<option value="' . $opt_value . '"' . $selected . '>' . $opt_label . '</option>';
		}
			...
			if ( ! in_array( $value, $valid_values, true ) ) {
				return new \WP_Error(
					...
					'' !== $custom_error
						? esc_html( $custom_error )
						/* translators: %s: field label */
						: sprintf( __( '%s has an invalid selection.', 'wprobo-documerge' ), $field_data['label'] )
				);
			}
		}
