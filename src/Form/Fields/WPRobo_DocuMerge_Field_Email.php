<?php
/**
 * Email field type for WPRobo DocuMerge form builder.
 *
...
/**
 * Class WPRobo_DocuMerge_Field_Email
 *
 * Handles email input fields within the DocuMerge form builder.
 *
 * @since 1.0.0
 */
class WPRobo_DocuMerge_Field_Email {
	...
	public function wprobo_documerge_get_label() {
		return __( 'Email', 'wprobo-documerge' );
	}
	...
	public function wprobo_documerge_render_frontend( $field_data, $value = '' ) {
		...
		$html .= '</label>';
		$html .= '<input type="email" id="wdm-field-' . $id . '" name="' . $name . '" class="wdm-input" value="' . $value_attr . '" placeholder="' . $placeholder . '" ' . $required . '>';
	...
	public function wprobo_documerge_validate( $value, $field_data ) {
		...
		if ( '' !== $value && ! is_email( $value ) ) {
			return new \WP_Error(
				'wprobo_documerge_invalid_email',
				'' !== $custom_error
					? esc_html( $custom_error )
					/* translators: %s: field label */
					: sprintf( __( '%s must be a valid email address.', 'wprobo-documerge' ), $field_data['label'] )
			);
		}
