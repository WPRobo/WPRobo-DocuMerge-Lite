<?php
/**
 * Number field type for WPRobo DocuMerge form builder.
 *
...
/**
 * Class WPRobo_DocuMerge_Field_Number
 *
 * Handles number input fields within the DocuMerge form builder.
 *
 * @since 1.0.0
 */
class WPRobo_DocuMerge_Field_Number {
	...
	public function wprobo_documerge_render_frontend( $field_data, $value = '' ) {
		...
		$html .= '</label>';
		$html .= '<input type="number" id="wdm-field-' . $id . '" name="' . $name . '" class="wdm-input" value="' . $value_attr . '" placeholder="' . $placeholder . '" step="' . $step . '" ' . $min_attr . ' ' . $max_attr . ' ' . $required . '>';
	...
	public function wprobo_documerge_validate( $value, $field_data ) {
		...
		if ( '' !== $value ) {
			if ( ! is_numeric( $value ) ) {
				return new \WP_Error(
					'wprobo_documerge_invalid_number',
					'' !== $custom_error
						? esc_html( $custom_error )
						/* translators: %s: field label */
			...
			if ( '' !== $field_data['min_value'] && $num < floatval( $field_data['min_value'] ) ) {
				return new \WP_Error(
					'wprobo_documerge_min_value',
					'' !== $custom_error
						? esc_html( $custom_error )
			...
			if ( '' !== $field_data['max_value'] && $num > floatval( $field_data['max_value'] ) ) {
				return new \WP_Error(
					'wprobo_documerge_max_value',
					'' !== $custom_error
						? esc_html( $custom_error )
